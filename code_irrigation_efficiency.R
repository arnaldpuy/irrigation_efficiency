...
a <- bos2 %>%
  ...
  scale_fill_manual(values = c("#D16103", "#C3D7A4")) +
...
## ----distr_final, cache=TRUE, dependson=c("distributions_func", "truncated_distr", "matrix_fun", "unc_large_fraction")----
...
## ----full_model, cache=TRUE, dependson=c("distr_final", "unc_large_fraction", "distributions_func", "truncated_distr", "matrix_fun")----
...
## ----run_model, cache=TRUE, dependson=c("distr_final", "unc_large_fraction", "distributions_func", "truncated_distr", "matrix_fun", "full_model", "settings")----
...
## ----cv_irrigation, cache=TRUE, dependson="extract_output", fig.height=6.4, fig.width=5.2, message=FALSE, warning=FALSE----
...
ranges_efficiencies <- fread("ranges_efficiencies.csv") %>%
  melt(., measure.vars = c("higher", "lower"), value.name = "V1") %>%
  .[, variable:= NULL]

ranges_empirical <- uncertainty.dt[, Study:= "This study"]%>%
  .[!IFT == "Jager"] %>%
  .[, .(IFT, Study, V1)]

rbind(ranges_empirical, ranges_efficiencies) %>%
  ...
  na.omit() %>%
  ggplot(., aes(V1, Study, color = ifelse(Study == "This study", "red", "black"))) +
...
## ----check_overlap, cache=TRUE, dependson="extract_output", fig.height=6.4, fig.width=5.2----
...
## ----plot_overlap_histogram, cache=TRUE, dependson="check_overlap", fig.height=2.5, fig.width=3.5, message=FALSE, warning=FALSE----
...
## ----correspondence_ift, cache=TRUE, dependson="check_overlap"----------------------
...
## ----unc_analysis2, cache=TRUE, dependson="extract_output", fig.height=6.4, fig.width=5.2----
...
## ----define_factor_unc, cache=TRUE, dependson="extract_output", fig.height=1.5, fig.width=5.6----
...
isimip.quantiles <- isimip.dt[, .(quantile.low = quantile(Water.Withdrawn, 
                                                          prob = 0.025, na.rm = TRUE),
                                  quantile.high = quantile(Water.Withdrawn, 
                                                           prob = 0.975, na.rm = TRUE)),
                              .(Continent, Country)] %>%
  .[, Approach:= "GMs"]

isimip.dt.unc.efficiencies <- merge(efficiency.dt, isimip.dt, 
                                    by = c("Country", "Continent"), all.x = TRUE,
                                    allow.cartesian = TRUE) %>%
  .[, Water.Withdrawn:= Water.Withdrawn / Ep] %>%
  .[, .(quantile.low = quantile(Water.Withdrawn, prob = 0.025, na.rm = TRUE),
    quantile.high = quantile(Water.Withdrawn, prob = 0.975, na.rm = TRUE)),
    .(Continent, Country)] %>%
  .[, Approach:= "GMs + uncertainty in irrigation efficiency"]


## ----climate_uncertainties, cache=TRUE----------------------------------------------
...
## ----climate_uncertainties_arrange, cache=TRUE, dependson="climate_uncertainties"----
...
# Check global IWW estimates per scenario and model
isimip.climate.dt[, .(total = sum(Water.Withdrawn, na.rm = TRUE)), 
                  .(Model, `Climate scenario`)]


## ----plot_unc_comparison, cache=TRUE, dependson=c("merge_isimip_data", "climate_uncertainties_arrange"), dev = "pdf", fig.height=4.2, fig.width=4.5----
...
isimip.climate <- isimip.climate.dt[, .(quantile.low = quantile(Water.Withdrawn, prob = 0.025, na.rm = TRUE),
                                        quantile.high = quantile(Water.Withdrawn, prob = 0.975, na.rm = TRUE)),
                                    .(Continent, Country)] %>%
  .[, Approach:= "GMs + uncertainty in climate change"]

all.uncertainties <- rbind(isimip.quantiles,
                           isimip.dt.unc.efficiencies,
                           isimip.climate) %>%
  .[, mean:= (quantile.low + quantile.high) / 2] %>%
  na.omit()
...
all.uncertainties %>%
  ...
  geom_errorbar(aes(ymin = quantile.high,
                    ymax = quantile.low),
...
## ----plot_unc_comparison_complete, cache=TRUE, dependson=c("merge_isimip_data", "plot_unc_merge"), fig.height=7, fig.width=5.2, dev = "pdf", message=FALSE, warning=FALSE----

# PLOT RANGES OF STRUCTURAL UNCERTAINTY AND RANGES OF 
# STRUCTURAL UNCERTAINTY + UNCERTAINTY IN IRRIGATION EFFICIENCY (COMPLETE) -------

vec1 <- all.uncertainties[Approach == "GMs", Country]
vec2 <- all.uncertainties[Approach == "GMs + uncertainty in climate change", Country]
vec3 <- all.uncertainties[Approach == "GMs + uncertainty in irrigation efficiency", Country]
...
## ----print_ranges, cache=TRUE, dependson="plot_unc_comparison", fig.height=2.5, fig.width=2.7----

# COMPARE RANGES -----------------------------------------------------------------

all.uncertainties <- all.uncertainties[, range:= quantile.high - quantile.low]

# Number of countries with larger uncertainties in irrigation efficiencies than climate change
# Number of countries with higher values in irrigation efficiency than climate change
measures <- c("range", "quantile.high")

out <- lapply(measures, function(measures)
  dcast(all.uncertainties, Continent + Country ~ Approach, value.var = measures) %>%
    .[, larger.uncertainties.efficiency:= ifelse(`GMs + uncertainty in irrigation efficiency` >
                                                   `GMs + uncertainty in climate change`, "Yes", "No")] %>%
    .[, .(N = .N), larger.uncertainties.efficiency])

  out

  # Check countries
lapply(measures, function(measures)
  dcast(all.uncertainties, Continent + Country ~ Approach, value.var = measures) %>%
    .[, larger.uncertainties.efficiency:= ifelse(`GMs + uncertainty in irrigation efficiency` >
                                                   `GMs + uncertainty in climate change`, "Yes", "No")] %>%
  .[larger.uncertainties.efficiency == "No"])

countries <- c("India", "Mexico", "Egypt", "Spain")
all.uncertainties[Country %in% countries] %>%
  .[order(Country)]

# Plot differences in factors
all.uncertainties %>%
  na.omit() %>%
  dcast(., Continent + Country ~ Approach, value.var = "quantile.high") %>%
  .[, diff:= `GMs + uncertainty in irrigation efficiency` /
      `GMs + uncertainty in climate change`] %>%
  .[order(-diff)] %>%
  na.omit() %>%
  ggplot(., aes(diff)) +
  geom_histogram() + 
  facet_wrap(~Continent) +
  labs(x = "Factor", y = "Nº of countries") +
  theme_AP()
...
## ----plot_sobol, cache=TRUE, dependson=c("extract_sobol"), fig.height=5, fig.width=2.5, dev = "pdf"----
                                              ...
                                              ifelse(parameters == "Ea_micro", "E[a]",
                                                     ifelse(parameters == "Ec_micro", "E[c]",
                                                            ifelse(parameters == "X1", "X[1]",
                                                                   ifelse(parameters == "X2", "X[2]",
                                                                          ifelse(parameters == "r_L", "r[L]", parameters)))))))))]
...
## ----extract_jager_indices, cache=TRUE, dependson="extract_output", fig.height=8, fig.width=5.5, dev = "pdf"----
...
## ----jager_merged, cache=TRUE, dependson="extract_jager_indices", dev = "pdf"-------
...
## ----merge_indices, cache=TRUE, dependson=c("extract_jager_indices", "plot_sobol"), dev = "pdf", fig.height=3.2, fig.width=5.5----
